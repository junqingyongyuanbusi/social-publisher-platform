...
@ApiBearerAuth()
@Controller('identity')
class IdentityWorkspacesController {
  @Get('workspaces')
  @ApiOperation({ summary: 'List workspace memberships from the verified access token' })
  list(@Req() request: AuthenticatedRequest) {
    return getPrincipal(request).memberships;
  }
}

@Module({ controllers: [IdentityController, IdentityWorkspacesController] })
